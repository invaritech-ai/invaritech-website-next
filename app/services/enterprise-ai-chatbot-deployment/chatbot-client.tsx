...
import { ArrowLeft, Check, ArrowRight, X, Clock, ShieldCheck, Zap, User, Lock, Globe, MessageSquare, AlertTriangle, Fingerprint, Activity } from "lucide-react";
    ...
    return (
        <main ref={containerRef} className="min-h-screen bg-background relative overflow-hidden">
            ...
            <div className="absolute top-24 left-6 z-20 md:top-32 md:left-12 hero-content">
                <Link href="/services/" className="group flex items-center text-muted-foreground hover:text-purple-400 transition-colors text-sm uppercase tracking-wider font-mono">
            ...
            <section className="pt-40 pb-20 px-6 relative z-10 hero-content">
                <div className="max-w-5xl mx-auto">
                    <Badge variant="outline" className="mb-6 border-purple-500/50 text-priority px-4 py-1 text-xs uppercase tracking-widest text-purple-400">
                        Service Offering
                    </Badge>
                        ...
                        <div>
                            <ul className="space-y-6 stagger-list">
                                {[
                                    ...
                                    <li key={i} className="stagger-item flex gap-4 items-center">
             ...
             <section className="reveal-section py-24 px-6 relative z-10">
                <div className="max-w-5xl mx-auto">
                    <div className="bg-red-900/10 border border-red-500/20 rounded-3xl p-8 md:p-12">
                         <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center text-red-50">Why Most Fail</h2>
                         <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 stagger-list">
                            {[
                                { title: "No Defined Objective", desc: "So nobody owns success." },
                                { title: "No RBAC/ABAC", desc: "Everyone can see everything." },
                                { title: "No Traceability", desc: "No one can explain what happened." },
                                { title: "Blind Retrieval", desc: "Context injection without boundaries." },
                                { title: "No Adoption Model", desc: "Middle management rejects it." }
                            ].map((item, i) => (
                                <div key={i} className="stagger-item p-6 border border-red-500/10 rounded-xl bg-black/40 hover:bg-red-900/20 transition-colors">
                                    <X className="w-6 h-6 text-red-500 mb-4" />
                                    <h3 className="font-bold text-white mb-2">{item.title}</h3>
                                    <p className="text-sm text-red-200/60">{item.desc}</p>
                                </div>
                            ))}
                         </div>
                    </div>
            ...
            <section className="reveal-section py-24 px-6 relative z-10 border-t border-white/10 bg-black/20">
                <div className="max-w-5xl mx-auto">
                     <div className="text-center mb-16">
                        ...
                        <h2 className="text-3xl md:text-5xl font-bold mb-6">What “Enterprise” Means to Us</h2>
                     ...
                     <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 stagger-list">
                        {[
                            { title: "RBAC / ABAC", desc: "Least-privilege access to data and tools.", icon: Lock },
                            { title: "Tool Boundaries", desc: "Deterministic tool usage (no blind execution).", icon: Activity },
                            { title: "Citations", desc: "Traceable answers with source references.", icon: MessageSquare },
                            { title: "Audit Logs", desc: "Queries, sources, actions, and outputs logged.", icon: Fingerprint },
                            { title: "Escalation Paths", desc: "Human review for high-risk actions.", icon: User },
                            { title: "Monitoring", desc: "Evaluation and drift detection over time.", icon: Activity }
                        ].map((item, i) => (
                        ...
                        {/* Valid Objective */}
                         <div className="bg-white/5 border border-white/10 rounded-2xl p-8">
                            <h3 className="text-2xl font-bold mb-6 flex items-center gap-3">
                                <Check className="w-6 h-6 text-primary" /> Valid Objectives
                            </h3>
                            <ul className="space-y-4">
                                {[
                                    ...
                                    <li key={i} className="flex gap-3 text-muted-foreground border-b border-white/5 pb-3 last:border-0 last:pb-0">
                         ...
                         <div className="bg-white/5 border border-white/10 rounded-2xl p-8">
                            <h3 className="text-2xl font-bold mb-6 flex items-center gap-3">
                                <User className="w-6 h-6 text-primary" /> Adoption Reality
                            </h3>
                            ...
                            <div className="bg-black/40 rounded-xl p-6 border border-white/5">
                                <h4 className="font-bold text-white mb-3 text-sm uppercase tracking-widest">Multilingual Deployment</h4>
                                <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
                                    <span className="px-3 py-1 bg-white/5 rounded-full border border-white/10">English</span>
                                    <span className="px-3 py-1 bg-white/5 rounded-full border border-white/10">Cantonese</span>
                                    <span className="px-3 py-1 bg-white/5 rounded-full border border-white/10">Mandarin</span>
                                    <span className="px-3 py-1 bg-white/5 rounded-full border border-white/10">Bahasa</span>
                                </div>
                                <p className="mt-4 text-xs text-white/50 italic">Language is identity. Governed AI should respect that.</p>
                            </div>
                        </div>
                    </div>
                </div>
             ...
             <section className="reveal-section py-24 px-6 relative z-10 border-t border-white/10 bg-black/20 backdrop-blur-sm">
                <div className="max-w-5xl mx-auto text-center">
                    <h2 className="text-3xl md:text-5xl font-bold mb-8">No Forced Lock-In</h2>
                    ...
                    <div className="grid md:grid-cols-3 gap-6 stagger-list">
                         <div className="stagger-item p-6 border border-white/10 rounded-xl bg-black/40">
                             <h4 className="font-bold text-white mb-2">Host Yourself</h4>
                             <p className="text-sm text-muted-foreground">VPC or on-prem where required.</p>
                         </div>
                         <div className="stagger-item p-6 border border-white/10 rounded-xl bg-black/40">
                             <h4 className="font-bold text-white mb-2">Approved Models</h4>
                             <p className="text-sm text-muted-foreground">Use providers you already trust.</p>
                         </div>
                         <div className="stagger-item p-6 border border-white/10 rounded-xl bg-black/40">
                             <h4 className="font-bold text-white mb-2">Cost Visibility</h4>
                             <p className="text-sm text-muted-foreground">Cost-per-task, not mystery bills.</p>
                         </div>
                    </div>
                </div>
            </section>
             ...
             <section className="reveal-section py-32 px-6 relative z-10">
                <div className="max-w-5xl mx-auto text-center">
                    <h2 className="text-4xl md:text-6xl font-bold mb-8 tracking-tighter mix-blend-difference text-white">
                        One Objective. Clear Boundaries. <br/> <span className="text-primary">Go / No-Go.</span>
                    </h2>
