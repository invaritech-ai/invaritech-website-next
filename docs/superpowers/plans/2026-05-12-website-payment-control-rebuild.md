...
**Architecture:** Replace the old broad AI homepage/service structure with a small set of focused App Router pages. Keep the existing Next.js/Tailwind/component stack, but introduce focused page components for homepage, resources, and contact capture so the public site reads as one coherent offer. Use `next.config.ts` redirects for legacy URLs and keep sitemap/structured data aligned with the reduced IA.
...
### Task 1: Align Shared Marketing Constants
...
### Task 2: Rebuild Header And Footer Navigation
...
```tsx
<span className="text-[10px] uppercase tracking-[0.3em] text-primary/90 font-bold text-center">Payment Controls</span>
```
...
### Task 3: Build Homepage From Approved Design
...
```tsx
<h1>Clean up payment controls without changing systems.</h1>
<p>We help finance teams reduce manual exception chasing, tighten approvals, and reduce dollar leakage across the software they already use.</p>
<p>Founder-led. One client at a time. Fixed-scope first, then managed support.</p>
```
...
<a href={BOOK_MEETING_URL} target="_blank" rel="noopener noreferrer">Book a Scoping Call</a>
```
...
### Task 4: Build Resources Page And Rule Table Form
...
### Task 8: Update Metadata, Sitemap, And Structured Data
...
### Task 9: Add Redirects And Remove Legacy Page Routes
...
### Task 10: QA Build, Browser Check, And Polish
...
Expected: No matches in kept public pages or shared shell. Matches in deleted files should not exist after Task 9.
...
- Homepage rewrite: Task 3.
- Resources and lead capture: Task 4.
- Contact retained and rebuilt: Task 5.
- About founder-led model: Task 6.
- Blog operational playbooks: Task 7.
- Nav/footer: Task 2.
- Metadata/sitemap/schema: Task 8.
- Aggressive cleanup and redirects: Task 9.
...
- `BOOK_MEETING_URL`, `BOOK_MEETING_CTA`, `RULE_TABLE_CTA`, and `BRAND_EYEBROW` are defined in Task 1 and used consistently in later tasks.
