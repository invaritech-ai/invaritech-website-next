...
exports.calculateHoursSaved = calculateHoursSaved;
exports.calculateCostAvoidance = calculateCostAvoidance;
function calculateAssessmentScore(inputs, config = {}) {
    const reasoning = [];
    const nextSteps = [];
    ...
    switch (inputs.currentAHTBand) {
        case "5+ min":
            viabilityRaw += 25;
            reasoning.push("Complex, lengthy tasks are prime candidates for AI optimization.");
            break;
        case "3-5 min":
            viabilityRaw += 20;
            break;
    ...
    if (goalScore.reason)
        reasoning.push(goalScore.reason);
    // Budget (Max 20)
    ...
    switch (inputs.processMaturity) {
        case "documented":
            readinessRaw += 30;
            reasoning.push("Documented SOPs significantly accelerate implementation speed.");
            break;
        case "partial":
            readinessRaw += 15;
            nextSteps.push("Formalize process documentation for your primary workflow.");
            break;
        case "tribal": // In heads
        default:
            readinessRaw += 5;
            reasoning.push("Tribal knowledge processes require a 'Discovery & Definition' phase first.");
    ...
    switch (inputs.dataStructure) {
        ...
        default:
            readinessRaw += 0;
            reasoning.push("Centralizing dispersed data sources will be a necessary first step.");
...
// Helpers
function calculateHoursSaved(inputs, readinessScore) {
...
function getWorkflowSavingsParams(primaryWorkflowGoal) {
...
function getAverageVolume(band) {
    switch (band) {
